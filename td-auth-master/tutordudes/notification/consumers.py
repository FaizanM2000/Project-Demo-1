...
from ..private_chat.models import UnreadChatMessage
from ..private_chat.utils import ClientError
...
class NotificationConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content, **kwargs):
        ...
        try:
            if command == "get_general_notifications":
                ...
                if payload == None:
                    raise ClientError(422, "Something went wrong. Try refreshing the browser.")
                else:
                    payload = json.loads(payload)
                    await self.send_updated_friend_request_notification(payload['notifications'])
    ...
    async def send_updated_friend_request_notification(self, notification):
        """
        After a friend request is accepted or declined, send the updated notifications to template
    ...
    async def send_chat_notifications_payload(self, notifications, new_page_number):
        ...
        """
        # print("NotificationConsumer: send_chat_notifications_payload", notifications, new_page_number)

        await self.send_json(
            {
                "chat_msg_type": CHAT_MSG_TYPE_NOTIFICATIONS,
                "notifications": notifications,
                "new_page_number": new_page_number,
            },
        )

    async def send_new_chat_notifications(self, notifications):
        """
        Called by receive_json when ready to send a json array of the notifications
        """
        await self.send_json(
...
def get_general_notifications(user, page_number):
    ...
    if user.is_authenticated:
        ...
        raise ClientError(422, "User must be authenticated to get notifications.")
    ...
    if user.is_authenticated:
        try:
            ...
            if friend_request.receiver == user:
                # accept the request and get the updated notifications
                updated_notification = friend_request.accept()

                # return the notifications associated with this FriendRequest
                s = LazyNotificationEncoder()
                payload['notifications'] = s.serialize([updated_notification])[0]
                return json.dumps(payload)
        except Notification.DoesNotExist:
            raise ClientError(422, "An error occurred with that notifications. Try refreshing the browser.")
    return None
...
def get_chat_notifications(user, page_number):
    ...
    if user.is_authenticated:
        content_type = ContentType.objects.get_for_model(UnreadChatMessage)
        notifications = Notification.objects.filter(target=user, content_type=content_type).order_by('-timestamp')
        p = Paginator(notifications, DEFAULT_NOTIFICATION_PAGE_SIZE)
...
def get_new_chat_notifications(user, newest_timestamp):
    ...
    if user.is_authenticated:
        ...
        timestamp = datetime.strptime(timestamp, '%Y-%m-%d %H:%M:%S.%f')
        content_type = ContentType.objects.get_for_model(UnreadChatMessage)
        notifications = Notification.objects.filter(target=user, content_type__in=[content_type],
                                                    timestamp__gt=timestamp).order_by('-timestamp')
        s = LazyNotificationEncoder()
        payload['notifications'] = s.serialize(notifications)
        return json.dumps(payload)
    else:
        raise ClientError(403, "User must be authenticated to get notifications.")
    return None
